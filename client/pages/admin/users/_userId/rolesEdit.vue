                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <i class="ca ca-anchor"></i>Rollenverteilung
                            <button class="btn btn-sm btn-info c-white float-right" type="submit">Speichern</button>
                        </div>
                        <div class="card-body">
                            <div class="form-group row"><!--v-for="(item, index) in user.role" :key="'role'+index"-->
                                <div class="select-wrapper col-8 mb-3">
                                    <select class="form-control  float-left" id="role" v-model="user.role">
                            ...
                            <!-- button type="button" class="btn btn-sm btn-success ml-1" v-on:click="addForm()">Rolle hinzufügen</button-->
                         </div>
                      </div>
                    </div>
                </div>
...
<script>
  export default {
    ...
    data () {
      return {
        index: Math.min(Math.max(parseInt(Number), 1), 3)
      }
    },
    methods: {
      save () {
